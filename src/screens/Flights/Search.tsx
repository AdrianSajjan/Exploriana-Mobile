...
type NavigationProps = NativeStackNavigationProp<AppStackParamList, "Search-Flights">;
...
export function SearchFlightScreen() {
  ...
  const { values, errors, touched, handleBlur, handleSubmit, handleChange, setFieldValue } = useFormik<FormState>({
    ...
    onSubmit: (values: FormState) => {
      ...
      navigation.navigate("Book-Flights");
    },
  });
    ...
    <SafeAreaView style={sharedStyles.fullHeight}>
      ...
      <ScrollView contentContainerStyle={[sharedStyles.fullGrow, sharedStyles.pvSmall, sharedStyles.ph]}>
        ...
        <Box marginTop={28}>
          ...
          <Body style={sharedStyles.headerDescription}>Search flights across various routes</Body>
        </Box>
        <Box marginTop={20}>
          <ArrivalDepartureInput
            icon={<Flight height={24} width={24} />}
          ...
          <PrimaryButton label="Search Flights" style={styles.button} onPress={() => handleSubmit()} />
        </Box>
      </ScrollView>
    </SafeAreaView>
  );
